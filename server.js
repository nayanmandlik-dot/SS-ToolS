...
// CORS allowlist. Defaults cover common static hosts; CORS_ORIGIN env var (comma-separated or '*') adds more.
const corsAllowExplicit = (process.env.CORS_ORIGIN || '').split(',').map((s) => s.trim()).filter(Boolean);
function isOriginAllowed(origin) {
  if (!origin) return true; // same-origin fetches and curl/server-to-server requests
  if (corsAllowExplicit.includes('*')) return true;
  if (corsAllowExplicit.includes(origin)) return true;
  try {
  ...
  } catch { /* malformed Origin header — fall through to deny */ }
  return false;
}
...
// Health check — useful for Render / uptime monitors / smoke tests.
